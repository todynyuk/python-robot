...
ComparisonWithoutItemsTest
    [Tags]  ComparisonWithoutItems
    launchingBrowser    ${URL}   ${BROWSER_CHROME}
    element should be enabled    ${comparison_link}
    login_Service
    click element    ${comparison_link}
    element should be enabled    ${no_one_items_message}
...
WishlistTest
    ...
    element should be enabled    ${no_items_wishlist_text}
    click element    ${header_logo_link}
    click_universal_category_link   ${SMARTPHONES_CATEGORY}
    click_universal_subcategory_menu_link   ${SMARTPHONES_SUBCATEGORY}
    clickCompareButtonByIndex    1
